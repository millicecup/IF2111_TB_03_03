...
void handleStoreMenu(UserList *userList, BarangList *barangList) {
    ...
    while (isInStoreMenu) {
        ...
        if (isWordEqualToString(&currentWord, "STORELIST")) {
        ...
        } else if (isWordEqualToString(&currentWord, "STOREREQUEST")) {
            printf("Feature: Store request is under construction.\n");
        } else if (isWordEqualToString(&currentWord, "STOREREMOVE")) {
            printf("Feature: Store remove is under construction.\n");
        } else if (isWordEqualToString(&currentWord, "STORESUPPLY")) {
            printf("Feature: Store supply is under construction.\n");
...
void handleInsideMenu(UserList *userList, BarangList *barangList) {
    char command[COMMAND_MAX_LEN];
    ...
    while (isInside) {
        ...
        if (isWordEqualToString(&currentWord, "STORE")) {
            handleStoreMenu(userList, barangList);
...
void handleLoginMenu(UserList *userList, BarangList *barangList) {
    char command[COMMAND_MAX_LEN];
    ...
    while (isInLoginMenu) {
        ...
        if (isWordEqualToString(&currentWord, "LOGIN")) {
            ...
            if (loggedIn) { // Assume loggedIn is set in login()
                handleInsideMenu(userList, barangList);
            }
...
void handleMainMenu(UserList *userList, BarangList *barangList, boolean *running) {
    char command[COMMAND_MAX_LEN];
    ...
    while (*running) {
        ...
        if (isWordEqualToString(&currentWord, "START")) {
            if (start(userList, barangList) == 0) {
                ...
                handleLoginMenu(userList, barangList);
            } else {
...
int main() {
    ...
    // Main loop
    while (running) {
        handleMainMenu(&userList, &barangList, &running);
    }
