...
void WishlistSwap(UserList *userlist, int i, int j){
    ...
    int n = NbWishlist(userlist->users->wishlist);
    if (i < 1 || j < 1 || i > n || j > n) {
    ...
    address p1 = First(userlist->users->wishlist);
    address p2 = First(userlist->users->wishlist);
    int idx;
    ...
    printf("Berhasil menukar posisi %s dengan %s pada userlist->users->wishlist!\n", Item(p1), Item(p2));
}
